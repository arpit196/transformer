...
def multihead_attention(queries, keys, values, key_masks,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        Q2 = tf.layers.dense(queries, d_model, use_bias=True) # (N, T_q, d_model)
        #K = tf.layers.dense(keys, d_model, use_bias=True) # (N, T_k, d_model)
        K1 = tf.layers.dense(keys, d_model, use_bias=True) # (N, T_q, d_model)
        K2 = tf.layers.dense(keys, d_model, use_bias=True) # (N, T_q, d_model)
        #V = tf.layers.dense(values, d_model, use_bias=True) # (N, T_k, d_model)
        V1 = tf.layers.dense(values, d_model, use_bias=True) # (N, T_q, d_model)
        V2 = tf.layers.dense(queries, d_model, use_bias=True) # (N, T_q, d_model)
        ...
        K2_ = tf.concat(tf.split(K2, num_heads, axis=2), axis=0) # (h*N, T_q, d_model/h)
        V1_ = tf.concat(tf.split(V1, num_heads, axis=2), axis=0) # (h*N, T_k, d_model/h)
        V2_ = tf.concat(tf.split(V2, num_heads, axis=2), axis=0) # (h*N, T_q, d_model/h)
        ...
        output =tf.concat([outputs1,outputs2],axis=1)
